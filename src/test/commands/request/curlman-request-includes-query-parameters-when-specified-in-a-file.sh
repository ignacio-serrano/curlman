...
echo 'query:since' > "$theServiceDir/users/GET"
echo 'query:since=46' > "$tmpDir/request.parameters"

pushd "$theServiceDir/users" > /dev/null

# ACT
$curlman_dev_home/src/main/curlman.sh request ./GET "$tmpDir/request.parameters" --show-command > "$tmpDir/out.txt"
exitCode=$?
popd > /dev/null
